...
function LoginPage({clientToken}) {
  ...
  const capturePhoto = () => {
    const imageSrc = webcamRef.current.getScreenshot({width: 704, height: 800})
  ...
  const FirstView = () => {
    return (
      <div className='page-content'>
        ...
        <input ref={emailFieldRef} name='email' type='email' placeholder='Email or phone' className='ms-text-box form-control' />
  ...
  const SecondView = () => {
    ...
    <div className='page-content'>
      <p className='email-header'>
        <a className='back-arrow' onClick={goBack}><img src={BackArrow} className='back-arrow-icon' alt='back' /></a>
        <span>{email}</span>
      </p>
      <h2 className='sub-title'>Authentication options</h2>
      <button className='authentication-option' onClick={goNext}>
        <img src={`data:image/jpeg;base64,${passwordIcon}`} className='auth-option-icon' alt='password' />
        <span className='auth-option-label'>Password</span>
      </button>
      <button className='authentication-option'>
        <img src={`data:image/jpeg;base64,${smartCardIcon}`} className='auth-option-icon' alt='smart or pin' />
  ...
  const FourthView = () => {
    return (
    <div className='page-content'>
      <p className='header-label'>For security reasons, we require additional information to verify your account</p>
      <p className='email-header'>
        <a className='back-arrow' onClick={goBack}><img src={BackArrow} className='back-arrow-icon' alt='back' /></a>
        <span>{email}</span>
      </p>
      <h2 className='sub-title'>Authentication options</h2>
      <button className='authentication-option'>
        <img src={`data:image/jpeg;base64,${smartCardIcon}`} className='auth-option-icon' alt='password' />
        <span className='auth-option-label'>Sign in with PIN or smartcard</span>
      </button>
      <button className='authentication-option'>
        <img src={`data:image/jpeg;base64,${tokenIcon}`} className='auth-option-icon' alt='smart or pin' />
  ...
  const SixthView = () => {
    ...
    <div className='page-content'>
      ...
      <img src={photoData} className='captured-image' alt='back' />
      <div className='button-bar' style={{marginTop:20}}>
  ...
  const EighthView = () => {
    ...
    <div className='page-content'>
      ...
      <h2 className='sub-title' style={{color: approvalState ? '#7FBA00' : '#cd0000' }}>Request for access has been {approvalState ? 'approved' : 'rejected'}</h2>
    </div>
    )
  }

  const transitions = useTransition(currentStep, null, {
    from: {opacity: 0, transform: isForwardAnim ? `translate(300px, 0px)` : `translate(-300px, 0px)`, position: 'absolute'},
    enter: {opacity: 1, transform: `translate(0px, 0px)`, position: 'relative'},
    leave: {opacity: 0, transform: isForwardAnim ? `translate(-300px, 0px)` : `translate(300px, 0px)`, position: 'absolute'},
  })
  ...
  const ViewGenerator = ({item}) => {
    switch (item) {
      ...
      case 4: return <FifthView />
      case 5: return <SixthView />
      case 6: return <SeventhView />
      case 7: return <EighthView />
    ...
    <div style={{backgroundImage:`url(${BgImage})`}} className='signin-page'>
      ...
      <footer id='footer'>
        <div id='footerLinks' className='floatReverse'>
          <div><span id='copyright'>© 2021 Facial Barnacle</span><a id='helpDesk' className='pageLink footerLink' href='mailto:rowlandotirnoo@gmail.com'>Help &amp; Support</a></div>
        </div>
      </footer>
    </div>
  )
